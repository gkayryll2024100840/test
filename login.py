...
# ============================================================
# HEADER (rendered only when logged in)
# ============================================================
def render_navbar():
    """Renders the top header bar with breadcrumbs, title, and active program."""
    # Pull active program from session state; fall back to a neutral label
    program_code = st.session_state.get("active_program_code", "—")
...
<style>
    ...
    .pulse-navbar-content {
        display: flex;
        flex-direction: column;
        gap: 4px;
    }

    .pulse-navbar-breadcrumbs {
        font-size: 11px;
        font-weight: 600;
    ...
    .pulse-navbar-program {
        font-size: 14px;
        color: #f5d5d7;
    }

    .pulse-navbar-program strong {
        color: #ffffff;
        font-weight: 700;
    }

    /* Push Streamlit's own header below ours */
    [data-testid="stHeader"] {
        z-index: 100;
        background: transparent;
    }

    /* Keep sidebar above the navbar */
    [data-testid="stSidebar"] {
        z-index: 1000;
    }

    /* Add top padding so page content clears the fixed navbar */
    .block-container {
...
<nav class="pulse-navbar">
  <div class="pulse-navbar-content">
    <div class="pulse-navbar-breadcrumbs">MAPÚA UNIVERSITY · ASU PATHWAYS</div>
    <div class="pulse-navbar-title">ETYSB Dashboard — {program_code} Program</div>
  </div>
  <div class="pulse-navbar-program">
...
if not st.session_state.get('logged_in'):
    ...
    # Render the header (uses active_program_code from session state)
    render_navbar()
